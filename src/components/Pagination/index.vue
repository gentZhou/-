<template>
  <div class="pagination">
    ...
    <button v-for="page in startEnd.end" :key="page" v-if="page >= startEnd.start" :class="{active:currentPageNum === page}" @click="$emit('changePageNum',page)">{{ page }}</button>
...
export default {
  ...
  computed: {
    ...
    //2.连续页的起始和结束页码
    startEnd() {
      let { currentPageNum, totalPageNum, continueSize } = this;
      let start, end,disNum;
      if(totalPageNum <= continueSize){
        start = 1;
        end = totalPageNum
      }else{
        //正常情况
